...
def _csr_access(self, dut, addr, r_stb=0, r_data=0, w_stb=0, w_data=0):
...
class RxPeripheralTestCase(unittest.TestCase):
    ...
    def test_sim(self):
        ...
        def testbench():
            ...
            self.assertEqual((yield dut.phy.rst), 1)

            # - read Config (enable=0):
            yield from _csr_access(self, dut, config_addr, r_stb=1, r_data=0)

            self.assertEqual((yield dut.phy.config), 0xa455)

            # - read PhyConfig (=0xa455) and write 0x0001:
            yield from _csr_access(self, dut, phy_config_addr+0, r_stb=1, r_data=0x55, w_stb=1, w_data=0x01)
            self.assertEqual((yield dut.phy.config), 0xa455)
            yield from _csr_access(self, dut, phy_config_addr+1, r_stb=1, r_data=0xa4, w_stb=1, w_data=0x00)
            self.assertEqual((yield dut.phy.config), 0xa455)
            yield Tick()
            self.assertEqual((yield dut.phy.config), 0x0001)

            # - read PhyConfig (=0x0001):
            yield from _csr_access(self, dut, phy_config_addr+0, r_stb=1, r_data=0x01)
            yield from _csr_access(self, dut, phy_config_addr+1, r_stb=1, r_data=0x00)

            # - read Status (ready=0, overflow=0, error=0):
            yield from _csr_access(self, dut, status_addr, r_stb=1, r_data=0b000)
            ...
            yield from _csr_access(self, dut, config_addr, r_stb=1, r_data=0, w_stb=1, w_data=1)
            self.assertEqual((yield dut.phy.rst), 1)
            yield Tick()
            self.assertEqual((yield dut.phy.rst), 0)
            ...
            yield from _csr_access(self, dut, phy_config_addr+0, r_stb=1, r_data=0x01, w_stb=1, w_data=0x55)
            yield from _csr_access(self, dut, phy_config_addr+1, r_stb=1, r_data=0x00, w_stb=1, w_data=0xa4)
            yield Tick()
            self.assertEqual((yield dut.phy.config), 0x0001)
            ...
            self.assertEqual((yield dut.phy.symbols.ready), 0)

            yield dut.phy.symbols.payload.eq(ord("a"))
            yield dut.phy.symbols.valid.eq(1)

            # - read Status (ready=1, overflow=0, error=0):
            yield from _csr_access(self, dut, status_addr, r_stb=1, r_data=0b001)

            # - read Data (="a"):
            yield dut.bus.addr.eq(data_addr)
            yield dut.bus.r_stb.eq(1)
            yield Tick()
            self.assertEqual((yield dut.bus.r_data), ord("a"))
            self.assertEqual((yield dut.phy.symbols.ready), 1)
            yield dut.bus.r_stb.eq(0)
            yield Tick()
            self.assertEqual((yield dut.phy.symbols.ready), 0)
            yield dut.phy.symbols.valid.eq(0)

            # - read Status (ready=0, overflow=0, error=0):
            yield from _csr_access(self, dut, status_addr, r_stb=1, r_data=0b000)

            yield dut.phy.error.eq(1)
            yield dut.phy.overflow.eq(1)
            yield Tick()
            yield dut.phy.error.eq(0)
            yield dut.phy.overflow.eq(0)

            # - read Status (ready=0, overflow=1, error=1):
            yield from _csr_access(self, dut, status_addr, r_stb=1, r_data=0b110)

            yield dut.phy.symbols.payload.eq(ord("b"))
            yield dut.phy.symbols.valid.eq(1)

            # - read Status (ready=1, overflow=1, error=1):
            yield from _csr_access(self, dut, status_addr, r_stb=1, r_data=0b111)
            ...
            yield from _csr_access(self, dut, config_addr, r_stb=1, r_data=1, w_stb=1, w_data=0)
            self.assertEqual((yield dut.phy.rst), 0)
            yield Tick()
            self.assertEqual((yield dut.phy.rst), 1)
            ...
            yield from _csr_access(self, dut, status_addr, r_stb=1, r_data=0b110, w_stb=1, w_data=0b110)
            yield Tick()
...
class TxPeripheralTestCase(unittest.TestCase):
    ...
    def test_sim(self):
        ...
        def testbench():
            ...
            self.assertEqual((yield dut.phy.rst), 1)
            yield dut.phy.symbols.ready.eq(0)
            ...
            # - read Status (ready=0):
            yield from _csr_access(self, dut, status_addr, r_stb=1, r_data=0)
            ...
            yield dut.phy.symbols.ready.eq(1)

            # - read Status (ready=1)
            yield from _csr_access(self, dut, status_addr, r_stb=1, r_data=1)

            # - write "a" to Data:
            yield dut.bus.addr.eq(data_addr)
            yield dut.bus.w_stb.eq(1)
            yield dut.bus.w_data.eq(ord("a"))
            yield Tick()
            self.assertEqual((yield dut.phy.symbols.valid), 1)
            self.assertEqual((yield dut.phy.symbols.payload), ord("a"))
            yield dut.bus.w_stb.eq(0)
            yield Tick()
            self.assertEqual((yield dut.phy.symbols.valid), 0)
...
class PeripheralTestCase(unittest.TestCase):
    ...
    def test_sim(self):
        ...
        def testbench():
            ...
            yield from _csr_access(self, dut, rx_status_addr, r_stb=1, r_data=0b000)

            # - read tx.Status (ready=0):
            yield from _csr_access(self, dut, tx_status_addr, r_stb=1, r_data=0)

            # - write "x" to tx.Data:
            yield from _csr_access(self, dut, tx_data_addr, w_stb=1, w_data=ord("x"))
            yield Tick()
            ...
            yield from _csr_access(self, dut, rx_config_addr, r_stb=1, r_data=0, w_stb=1, w_data=1)
            yield Tick()

            # - read tx.Config (enable=0) and write 1:
            yield from _csr_access(self, dut, tx_config_addr, r_stb=1, r_data=0, w_stb=1, w_data=1)
            yield Tick()
            ...
            for c in "abcd":
                ...
                yield from _csr_access(self, dut, tx_status_addr, r_stb=1, r_data=1)

                # - write c to tx.Data:
                yield from _csr_access(self, dut, tx_data_addr, w_stb=1, w_data=ord(c))
                yield Tick()

            # - read rx.Status (ready=1, overflow=0, error=0):
            yield from _csr_access(self, dut, rx_status_addr, r_stb=1, r_data=0b001)

            # - read tx.Status (ready=0):
            yield from _csr_access(self, dut, tx_status_addr, r_stb=1, r_data=0)

            # - write "e" to tx.Data:
            yield from _csr_access(self, dut, tx_data_addr, w_stb=1, w_data=ord("e"))
            yield Tick()

            # - read rx.Status (ready=1, overflow=1, error=0) and write 0b010:
            yield from _csr_access(self, dut, rx_status_addr, r_stb=1, r_data=0b011, w_stb=1, w_data=0b010)
            yield Tick()
            ...
            for c in "abcd":
                ...
                # - read rx.Data (=c)
                yield from _csr_access(self, dut, rx_data_addr, r_stb=1, r_data=ord(c))
            ...
            yield from _csr_access(self, dut, rx_config_addr, r_stb=1, r_data=1, w_stb=1, w_data=0)
            yield Tick()

            # - read tx.Config (enable=1) and write 0:
            yield from _csr_access(self, dut, tx_config_addr, r_stb=1, r_data=1, w_stb=1, w_data=0)
            yield Tick()
