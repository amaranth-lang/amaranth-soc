...
class RxPhySignature(wiring.Signature):
    ...
    symbols : :class:`stream.Interface`
    ...
    def __init__(self, phy_config_shape, symbol_shape):
        super().__init__({
            ...
            "symbols":  In(wiring.Signature({ # TODO: replace with stream.Signature
                            "payload": Out(symbol_shape),
                            "valid":   Out(1),
                            "ready":   In(1),
                        })),
            "overflow": In(1),
...
class TxPhySignature(wiring.Signature):
    ...
    def __init__(self, phy_config_shape, symbol_shape):
        super().__init__({
            "rst":     Out(1),
            "config":  Out(phy_config_shape),
            "symbols": Out(wiring.Signature({ # TODO: replace with stream.Signature
                           "payload": Out(symbol_shape),
                           "valid":   Out(1),
                           "ready":   In(1),
                       })),
...
class RxPeripheral(wiring.Component):
    ...
    data_width : :class:`int`
        CSR bus data width.
    name : :class:`str`
        CSR bus window name. Optional. Defaults to ``None``.
    phy_config_shape : :ref:`shape-like <lang-shapelike>`
    ...
    phy : :class:`wiring.PureInterface` of :class:`RxPhySignature`
        Interface between the peripheral and its PHY.
    """
    def __init__(self, *, addr_width, data_width, name=None, phy_config_shape=unsigned(16),
...
class TxPeripheral(wiring.Component):
    ...
    bus : :class:`csr.Interface`
        CSR bus interface providing access to registers.
    phy : :class:`wiring.PureInterface` of :class:`TxPhySignature`
        Interface between the peripheral and its PHY.
    """
    def __init__(self, *, addr_width, data_width=8, name=None, phy_config_shape=unsigned(16),
                 phy_config_init=0, symbol_shape=unsigned(8)):
        regs = csr.Builder(addr_width=addr_width, data_width=data_width, name=name)
...
class Peripheral(wiring.Component):
    ...
    Both subordinates have the same parameters as this peripheral, except ``name`` (which becomes
    ``"rx"`` or ``"tx"`` respectively) and ``addr_width`` (which becomes ``addr_width - 1``).
    ...
    bus : :class:`csr.Interface`
        CSR bus interface providing access to registers.
    rx : :class:`wiring.PureInterface` of :class:`RxPhySignature`
        Interface between the peripheral and its PHY receiver.
    tx : :class:`wiring.PureInterface` of :class:`TxPhySignature`
    ...
    def __init__(self, *, addr_width, data_width=8, name=None, phy_config_shape=unsigned(16),
        ...
        self._rx = RxPeripheral(addr_width=addr_width - 1, data_width=data_width, name="rx",
                                phy_config_shape=phy_config_shape, phy_config_init=phy_config_init,
                                symbol_shape=symbol_shape)
        self._tx = TxPeripheral(addr_width=addr_width - 1, data_width=data_width, name="tx",
                                phy_config_shape=phy_config_shape, phy_config_init=phy_config_init,
                                symbol_shape=symbol_shape)

        self._decoder = csr.Decoder(addr_width=addr_width, data_width=data_width, name=name)
        self._decoder.add(self._rx.bus)
        self._decoder.add(self._tx.bus)
