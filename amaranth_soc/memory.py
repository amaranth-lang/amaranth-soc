...
class ResourceInfo:
    ...
    def path(self):
        ...
        Returns
        -------
        :class:`tuple` of (:class:`str` or (:class:`tuple` of :class:`str`))
        """
        return self._path
...
class MemoryMap:
    ...
    def windows(self):
        ...
        Yields
        ------
        :class:`tuple` of (:class:`MemoryMap`, :class:`tuple` of (:class:`int`, :class:`int`, \
        :class:`int`))
            A tuple ``window, (start, end, ratio)`` describing the address range assigned to
            the window. When bridging buses of unequal data widths, ``ratio`` is the amount of
    ...
    def window_patterns(self):
        ...
        Yields
        ------
        :class:`tuple` of (:class:`MemoryMap`, :class:`tuple` of (:class:`str`, :class:`int`))
            A tuple ``window, (pattern, ratio)`` describing the address range assigned to the
            window. ``pattern`` is a :attr:`addr_width` wide pattern that may be used in ``Case``
            or ``match`` to determine if an address signal is within the address range of
            ``window``. When bridging buses of unequal data widths, ``ratio`` is the amount of
            contiguous addresses on the narrower bus that are accessed for each transaction on
            the wider bus. Otherwise, it is always 1.
        """
